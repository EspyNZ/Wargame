...
export default function App() {
    ...
    for (const hexCoord in terrain) {
      ...
      switch (terrain[hexCoord]) {
        ...
        case 'town':
          ...
          break;
        default:
          terrainColor = '#dcfce7'; // Default plain terrain
          break;
      }
      drawHex(ctx, col, r, terrainColor, 0.5, '#fff');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-100 font-sans p-4 flex flex-col items-center">
      <div className="w-full max-w-7xl flex flex-col md:flex-row gap-4 mb-4">
        {/* Game Title and Controls */}
        <div className="flex-1 bg-gray-800 rounded-xl shadow-lg p-6 space-y-4">
          ...
          <div className="flex justify-center gap-4">
            <button
              ...
              className={`flex items-center gap-2 p-3 rounded-xl transition-all duration-300 ${
                showSetup ? 'bg-purple-600 text-white' : 'bg-gray-700 hover:bg-gray-600 text-gray-300'
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 p-3 rounded-xl transition-all duration-300 ${
                viewMode === 'player' ? 'bg-blue-600 text-white' : 'bg-gray-700 hover:bg-gray-600 text-gray-300'
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 p-3 rounded-xl transition-all duration-300 ${
                viewMode === 'umpire' ? 'bg-blue-600 text-white' : 'bg-gray-700 hover:bg-gray-600 text-gray-300'
              }`}
            >
            ...
            <button
              ...
              className="flex items-center gap-2 p-3 rounded-xl bg-green-600 hover:bg-green-500 text-white transition-all duration-300"
            >
      ...
      <div className="w-full max-w-7xl flex flex-col lg:flex-row gap-4">
        {showSetup ? (
          <div className="flex-1 bg-gray-800 rounded-xl shadow-lg p-6 flex flex-col items-center justify-center space-y-8">
            <h2 className="text-2xl font-bold text-blue-400">Battle Setup</h2>
            <p className="text-gray-400">As the Umpire, upload your map and force list to begin the battle.</p>
            
            {/* Map Upload Section */}
            <div className="bg-gray-700 rounded-xl p-6 w-full max-w-lg flex flex-col items-center space-y-4">
              <h3 className="text-xl font-semibold">1. Upload Map</h3>
              <p className="text-gray-300 text-sm text-center">Your custom map will be displayed as the background for the hex grid.</p>
              <label htmlFor="map-upload" className="flex items-center gap-2 px-6 py-3 rounded-xl bg-gray-600 hover:bg-gray-500 text-white transition-colors duration-300 cursor-pointer">
              ...
              <input
                ...
                className="hidden"
              />
              {mapImage && <p className="text-sm text-green-400">Map uploaded successfully!</p>}
            </div>

            {/* Units Upload Section */}
            <div className="bg-gray-700 rounded-xl p-6 w-full max-w-lg flex flex-col items-center space-y-4">
              <h3 className="text-xl font-semibold">2. Upload Force List (JSON)</h3>
              ...
              <label htmlFor="units-upload" className="flex items-center gap-2 px-6 py-3 rounded-xl bg-gray-600 hover:bg-gray-500 text-white transition-colors duration-300 cursor-pointer">
              ...
              <input
                ...
                className="hidden"
              />
              {units.length > 0 && <p className="text-sm text-green-400">{units.length} units loaded successfully!</p>}
            </div>

            <button
              onClick={() => setShowSetup(false)}
              className="px-8 py-4 bg-green-600 hover:bg-green-500 text-white text-lg font-bold rounded-xl transition-colors duration-300"
            >
          ...
          <>
            ...
            <div className="flex-1 bg-gray-800 rounded-xl shadow-lg p-6 flex flex-col items-center">
              <h2 className="text-xl font-semibold mb-4 text-center">Battle Map</h2>
              <div className="bg-gray-700 rounded-xl w-full p-2">
                <canvas ref={canvasRef} onClick={handleCanvasClick} className="w-full h-auto rounded-lg" />
              </div>
              {viewMode === 'umpire' && (
                <div className="mt-4 w-full bg-gray-700 rounded-xl p-4 space-y-4">
                  <h3 className="text-lg font-semibold text-center">Umpire Tools</h3>
                  {selectedHex && (
                    <div className="flex flex-col items-center justify-center space-y-2">
                      <p className="text-gray-300">Selected Hex: <span className="font-bold text-blue-400">{selectedHex}</span></p>
                      <div className="flex flex-wrap justify-center gap-2">
                        <button onClick={() => handleAssignTerrain('plain')} className="px-4 py-2 bg-green-600 hover:bg-green-500 text-white rounded-xl transition-colors duration-300">Set Plain</button>
                        <button onClick={() => handleAssignTerrain('hill')} className="px-4 py-2 bg-yellow-600 hover:bg-yellow-500 text-white rounded-xl transition-colors duration-300">Set Hill</button>
                        <button onClick={() => handleAssignTerrain('forest')} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl transition-colors duration-300">Set Forest</button>
                        <button onClick={() => handleAssignTerrain('river')} className="px-4 py-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded-xl transition-colors duration-300">Set River</button>
                        <button onClick={() => handleAssignTerrain('town')} className="px-4 py-2 bg-gray-600 hover:bg-gray-500 text-white rounded-xl transition-colors duration-300">Set Town</button>
                      </div>
                      <button onClick={handlePlaceUnit} className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl transition-colors duration-300"><PlusCircle size={16} className="inline-block mr-2" />Place New Unit</button>
                    </div>
                  )}
                </div>
              )}
              {viewMode === 'player' && (
                <div className="mt-4 w-full bg-gray-700 rounded-xl p-4 space-y-4">
                  <h3 className="text-lg font-semibold text-center">Player Orders</h3>
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="flex-1 p-3 rounded-xl bg-gray-900 text-gray-100 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors duration-300"
                    />
                    <button
                      onClick={() => {
                      ...
                      className="p-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl transition-colors duration-300"
                    >
            ...
            <div className="flex-1 bg-gray-800 rounded-xl shadow-lg p-6 space-y-6">
              <div className="bg-gray-700 rounded-xl p-4 space-y-2">
                <h2 className="text-xl font-semibold text-center mb-2">Unit Status</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-4">
                  {units.map(unit => {
                    const isVisible = viewMode === 'umpire' || unit.side === 'Blue' || unit.spotted;
                    if (!isVisible) return null;
                    return (
                      <div key={unit.id} className="bg-gray-900 rounded-xl p-4 shadow-inner border border-gray-600">
                        <div className="flex items-center gap-2 mb-2">
                          <div className={`w-3 h-3 rounded-full ${unit.side === 'Blue' ? 'bg-blue-500' : 'bg-red-500'}`} />
                          <h3 className="font-semibold text-lg">{unit.id}</h3>
                        </div>
                        <div className="grid grid-cols-2 gap-2 text-sm text-gray-400">
                          ...
                          <div className="flex items-center gap-1 col-span-2"><Scroll size={14} /> Orders: <span className="text-white truncate">{unit.orders || 'None'}</span></div>
                        </div>
                      </div>
                    );
                  })}
              ...
              <div className="bg-gray-700 rounded-xl p-4 space-y-2">
                <h2 className="text-xl font-semibold text-center mb-2">Turn Log</h2>
                <div className="h-48 overflow-y-auto bg-gray-900 rounded-lg p-3 border border-gray-600">
                  {turnLog.length === 0 ? (
                    <p className="text-gray-500 text-center text-sm">No events yet.</p>
                  ) : (
                    <ul className="space-y-1 text-sm text-gray-300">
                      {turnLog.map((log, index) => (
                        <li key={index} className="border-b border-gray-600 last:border-b-0 pb-1 mb-1">{log}</li>
                      ))}
                    </ul>
                  )}
                </div>
